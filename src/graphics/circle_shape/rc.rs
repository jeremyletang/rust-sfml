...
impl CircleShape {
    ...
    ///
    /// Return Some(CircleShape) or None
    pub fn new_init(radius: f32, point_count: usize) -> Option<CircleShape> {
        let circle = unsafe { ffi::sfCircleShape_create() };
        if circle.is_null() {
            None
        } else {
    ...
    pub fn get_point_count(&self) -> usize {
        unsafe {
            ffi::sfCircleShape_getPointCount(self.circle_shape) as usize
        }
    }
    ...
    pub fn get_point(&self, index: usize) -> () {
        unsafe {
