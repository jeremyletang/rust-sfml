...
unsafe extern "C" fn read<T: Read + Seek>(data: *mut c_void, size: c_longlong, user_data: *mut c_void) -> c_longlong {
...
unsafe extern "C" fn seek<T: Read + Seek>(position: c_longlong, user_data: *mut c_void) -> c_longlong {
    let stream: &mut T = &mut *(user_data as *mut T);
