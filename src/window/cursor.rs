...
impl Cursor {
    ...
    pub unsafe fn from_pixels(pixels: &[u8], size: Vector2u, hotspot: Vector2u) -> SfResult<Self> {
        let mut cursor = Cursor {
            _opaque: Default::default(),
        };
        let result =
            unsafe { sfCursor_createFromPixels(&mut cursor, pixels.as_ptr(), size, hotspot) }
                .into_sf_result();
    ...
    pub fn from_system(type_: Type) -> SfResult<Self> {
        let mut cursor = Cursor {
            _opaque: Default::default(),
        };

        let result = unsafe { sfCursor_createFromSystem(&mut cursor, type_) }.into_sf_result();
...
use self::ffi::{sfCursor_createFromPixels, sfCursor_createFromSystem};
